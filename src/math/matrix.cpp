...
double minor(const Matrix& m, size_t r, size_t c) { return determinant(m.submatrix(r, c)); }
...
double Matrix::max_abs() const {
    ...
    for (std::size_t r = 0; r < rows; ++r) {
        for (std::size_t c = 0; c < cols; ++c) {
            if (std::abs((*this)(r, c)) > max) max = std::abs((*this)(r, c));
        }
    }
    return max;
}
...
Matrix inverse_gauss_jordan(const Matrix& m) {
    if (m.rows != m.cols) throw std::runtime_error("Matrix must be square");
    ...
    for (size_t k = 0; k < n; ++k) {
        if (!check_pivot(augmented, k, eps)) {  // check every pivot
...
bool check_pivot(Matrix& m, size_t k, double eps) {
    ...
    size_t cols = m.cols;
    double max = 0.0;
    for (size_t r = k; r < rows; ++r) {  // r = pivot row
        double el = m(r, k);
        if (std::abs(el) > max) {  // at first pivot value is max, then look for larger values to minimize rounding error
            max = std::abs(el);
            row_with_max = r;
        }
    }
    if (max < eps) {  // no values found that meet threshold
        return false;
    }
    if (row_with_max == k) {  // pivot value was max
        return true;
    }
...
void normalize_row(Matrix& m, std::size_t k) {  // kth pivot
    std::size_t c = m.cols;
    double piv = m(k, k);
    for (std::size_t i = 0; i < c; ++i) {  // iterate through row
        double cur = m(k, i);
        m(k, i) = cur / piv;
    }
}
