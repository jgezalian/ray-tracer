...
Color lighting(const Shape* object, const Material &m, const Light &light, const Tuple &point, const Tuple &eyev,
    ...
    if(m.pattern != nullptr) {
        effective_color = m.pattern->pattern_at_object(object, point);
    }
    else effective_color = m.color * light.intensity;
